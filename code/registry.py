...
class SteamRegistry(object):
    ...
    def ajaxverifyemail(self, proxy):
        """
        请求creationid
        :return:
        """
    ...
    def email_active(self, proxy):
        ...
        while try_time > 0:
            try:
                time.sleep(8)
                system_log("已发送邮件，等待8秒钟取邮件信息...\n")
                email_info_url = '?address=' + self.email_address + '&password=' + self.email_pwd + '&pop3server=pop.126.com'
                email_response = requests.get('http://localhost:8888/parse_email' + email_info_url)
                if email_response.status_code == 200:
                    json_data = json.loads(email_response.text)
                    if json_data['status'] == 'ok':
                        system_log(json_data['msg'])
                        system_log('---' + email_info_url + '：：：' + json_data['msg'])
                        self.email_active_status = True
                        try_time = 0
                    else:
                        try_time = try_time - 1
                        system_log('---' + email_info_url + '：：：邮件解析，解析失败！')
                else:
                    try_time = try_time - 1
                    system_log('---' + email_info_url + '：：：邮件解析，解析失败！')
                ...
                # print(res.text)
                # system_log("[")
                # system_log(str(res.status_code) + ":::")
                # system_log(res.url)
                # system_log("]")
                # print('type:', self.type)
                # print('captchagid_gid:', self.captchagid_gid)
                ...
                # try_time = 0
                # self.email_active_status = True
                # system_log("解析邮件: " + self.email_address + "内容为..." + res.text + '\n')
                # system_log("解析邮件: " + self.email_address + "内容成功...开始注册...\n")
                # print('----------000---------')
                # # print(type(get_email_object.__str__()))
                # print('----------000---------')
                # senderRealName, senderAdr = email_parse.get_senderAdr()
                # if 'Steam' in senderRealName:
                ...
                #         system_log("解析邮件失败...开始第：" + str(try_time) + "尝试获取邮件：" + self.email_address + "内容")
                #         email_active_time -= 1
            except Exception as email_active_e:
                raise email_active_e

    def ajaxcheckemailverified(self, proxy):
        """
        请求和响应注册表单页面
        :return:
        """
